    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < figures.size(); i++) {
            ...
            if (barrierActivated) {
                if (figures.get(i).getyPosition() + figures.get(i).getMovement() > (RegularConstants.TRACK_HEIGHT / 2)
                    ...
                    if (RegularConstants.IMAGE_FIG) {
                        if (figures.get(i).getSpeed() == 10) {
                            try {
                                g.drawImage(ImageIO.read(new File("utility/bocho.jpg")), 0, 0, null);
                ...
                //Aqui se necesitan pintar las imagenes*************************************************************************************************
                if (RegularConstants.IMAGE_FIG == true) {
                    try {
                        g.drawImage(ImageIO.read(new File("/home/nelson/NetBeansProjects/Proyecto-hilos-lenguajes-tec/src/utility/bocho.jpg")),
                                RegularConstants.TRACK_WIDTH / 2 - RegularConstants.SIZE_FIGURE / 2, figures.get(i).getyPosition(), null);
                    } catch (IOException ex) {
                        Logger.getLogger(TrackPanel.class.getName()).log(Level.SEVERE, null, ex);
                    }
                } else {
